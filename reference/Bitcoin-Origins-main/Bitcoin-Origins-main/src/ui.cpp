...
void Shutdown(void* parg)
{
    ...
    CRITICAL_BLOCK(cs_Shutdown)
    {
        ...
        printf("Bitcoin Origins exiting\n");
        exit(0);
    }
}
...
CYourAddressDialog::CYourAddressDialog(wxWindow* parent, const string& strInitSelected) : CYourAddressDialogBase(parent)
{
    ...
    m_listCtrl->InsertColumn(1, "Bitcoin Origins Address", wxLIST_FORMAT_LEFT, 350);
    m_listCtrl->SetFocus();
...
void CYourAddressDialog::OnButtonRename(wxCommandEvent& event)
{
    ...
    CGetTextFromUserDialog dialog(this, "Rename Bitcoin Origins Address", "New Name", strName);
    if (!dialog.ShowModal())
        return;
...
{
    // Ask name
    CGetTextFromUserDialog dialog(this, "New Bitcoin Origins Address", "Name", "");
    if (!dialog.ShowModal())
        return;
...
void CViewProductDialog::GetOrder(CWalletTx& wtx)
{
    ...
    for (int i = 0; i < product.vOrderForm.size(); i++)
    {
        ...
        wtx.vOrderForm.push_back(
    make_pair(string(m_staticTextLabel[i]->GetLabel().mb_str()), strValue)
);
    }
}
...
bool CMyApp::OnInit2()
{
    ...
    //// debug print
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Bitcoin Origins CMyApp::OnInit()\n");

    //
    // Limit to single instance per user
    // Required to protect the database files if we're going to keep deleting log.*
    //
    wxString strMutexName = wxString("BitcoinOrigins.") + getenv("HOMEPATH");
    ...
    if (psingleinstancechecker->IsAnotherRunning())
    {
        ...
        loop
        {
            ...
            HWND hwndPrev = FindWindow("wxWindowClassNR", "Bitcoin Origins");
            if (hwndPrev)
            {
...
void ThreadRandSendTest(void* parg)
{
    ...
    if (!AddressToHash160(strAddress, hash160))
    {
        wxMessageBox(strprintf("ThreadRandSendTest: Bitcoin Origins address '%s' not valid ", strAddress.c_str()));
        return;
    }

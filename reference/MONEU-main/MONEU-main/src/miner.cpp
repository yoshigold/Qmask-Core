    ...
    if (mMempool) {
        ...
        for (const auto& tx : pending) {
            ...
            if (!ComputeTxFee(tx, fee)) {
                // ... leave original error handling here ...
                continue;
            }

            // 💰 QMASK 60/40 DEFLATIONARY FEE SPLITTER HOOK
            // 40% of the calculated transaction fee is immediately burned out of the circulating supply loop
            uint64_t burnAmount = static_cast<uint64_t>(fee * 0.40);
            fee = fee - burnAmount; // The remaining 60% stays as the active miner incentive reward
...
bool Miner::Start(const bytes32& coinbaseOutputHash, int threads)
{
    ...
    // ⚖️ QMASK THREAD THROTTLING FILTER INTERFACE
    // Compresses raw hardware processing cores via square-root scaling
    double baseThreads = static_cast<double>(threads);
    threads = static_cast<int>(std::sqrt(baseThreads) * 2.0); 
    if (threads < 1) threads = 1;
